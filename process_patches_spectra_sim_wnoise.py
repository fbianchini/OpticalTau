...
#Files
spectra_folder_5deg_sim_wnoise  = 'spectra_patches_radius5deg_sim_wnoise_gal080_MASTER_deltaell20'
spectra_folder_10deg_sim_wnoise = 'spectra_patches_radius10deg_sim_wnoise_gal080_MASTER_deltaell20'

spectra_folder_5deg_sim_wonoise  = 'spectra_patches_radius5deg_sim_wonoise_gal080_MASTER_deltaell20'
spectra_folder_10deg_sim_wonoise = 'spectra_patches_radius10deg_sim_wonoise_gal080_MASTER_deltaell20'

spectra_ref_sim_wnoise_file  = 'Dl_sim_wnoise_nopixwin_beam_ptsrc_MASTER_delta20.dat'
spectra_ref_sim_wonoise_file = 'Dl_sim_wonoise_nopixwin_beam_ptsrc_MASTER_delta20.dat'

l, dltt_ref_wnoise  = np.loadtxt(spectra_ref_sim_wnoise_file, unpack=1)
l, dltt_ref_wonoise = np.loadtxt(spectra_ref_sim_wonoise_file, unpack=1)
...
dls_5deg_wnoise   = GetDls(spectra_folder_5deg_sim_wnoise)
dls_10deg_wnoise  = GetDls(spectra_folder_10deg_sim_wnoise)
dls_5deg_wonoise  = GetDls(spectra_folder_5deg_sim_wonoise)
dls_10deg_wonoise = GetDls(spectra_folder_10deg_sim_wonoise)
...
fig.suptitle(r'DATA - Radius 5  deg - $N=%d$'%(dls_5deg_wonoise.shape[0]), size=15)
...
for iLM, LM in enumerate([1200,1900]):
	for ilm, lm in enumerate([300, 650, 1000]):
		...
		taus_wnoise  = GetManyTaus(dls_5deg_wnoise, dltt_ref_wnoise, l, lm, LM, method='CR')
		taus_wonoise = GetManyTaus(dls_5deg_wonoise, dltt_ref_wonoise, l, lm, LM, method='CR')

		# Random patches
		hist(taus_wnoise, 'knuth', histtype='step', ax=ax[iLM,ilm], label=r'w/  noise $\gamma_1=%.2f\pm%.2f$'%(stats.skew(taus_wnoise),err_skew(taus_wnoise)))
		hist(taus_wonoise,'knuth', histtype='step', ax=ax[iLM,ilm], label=r'w/o noise $\gamma_1=%.2f\pm%.2f$'%(stats.skew(taus_wonoise),err_skew(taus_wonoise)))
...
savefig('plots/taus_sim_w_wonoise_5deg.pdf', bboxes_inches='tight')
